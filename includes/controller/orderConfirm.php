...
class orderConfirm {
        ...
        if($customer_email){
           ...
           // $stmt = $conn->prepare($sql);
			...
			if($paid_status!='Paid' && ($pay_methode==3 || $pay_methode==4)){
				$subject = "#". $invoice_no." Order Confirmation from Roadhouse";
				...
				$subject = "#". $invoice_no." Order Paid Confirmation from Roadhouse";
    ...
    function paymentAdd($txnid, $payment_amount, $payment_status, $method, $itemid){
        ...
        try {
            ...
            if (strpos($item_number, 'RHG') !== false){
